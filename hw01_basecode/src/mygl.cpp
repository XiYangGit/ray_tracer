...
//        {
//            //TODO
//            Ray ray_ij=scene.camera.Raycast(i,j);
////            Intersection inter_ij=intersection_engine.GetIntersection(ray_ij);
////            scene.film.pixels[i][j]=glm::abs(inter_ij.normal);
//            unsigned int dep=0;;
//            scene.film.pixels[i][j]=integrator.TraceRay(ray_ij,dep);
//        }
//    }
    //render task
    int chunk_width_num=scene.camera.width/16;
    int wid_extra=15;
    int hgt_extra=15;
    if(chunk_width_num*16<scene.camera.width)
    {
        wid_extra=scene.camera.width-chunk_width_num*16;
        chunk_width_num++;
    }
    int chunk_height_num=scene.camera.height/16;
    if(chunk_height_num*16<scene.camera.height)
    {
        hgt_extra=scene.camera.height-chunk_height_num*16;
        chunk_height_num++;
    }

    for(int i=0;i<chunk_width_num;i++)
   {
        for(int j=0;j<chunk_height_num;j++)
        {
            glm::ivec2 min(i*16,j*16);
            glm::ivec2 max(i*16+15,j*16+15);
            if(i==chunk_width_num-1)
                max=glm::ivec2(i*16+wid_extra,j*16+15);
            if(j==chunk_height_num-1)
                max=glm::ivec2(i*16+15,j*16+hgt_extra);
            if((j==chunk_height_num-1)&&(i==chunk_width_num-1))
                max=glm::ivec2(i*16+wid_extra,j*16+hgt_extra);
